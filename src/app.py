...
def fallback_predict_energy(
    ...
    complexity = np.log1p(prompt_tokens) / 10.0
...
def mock_energy_curve_plot(num_layers: int, energy_kwh: float) -> plt.Figure:
    """Smaller mocked curve, like your original GUI, but compact."""
    data = pd.DataFrame({
    ...
    fig, ax = plt.subplots(figsize=(4, 3))  # smaller than before

    ax.plot(data["num_layers"], data["energy_kwh"], "o-", label="Actual")
    ax.plot(data["num_layers"], data["predicted"], "x--", label="Predicted")

    ax.scatter([num_layers], [energy_kwh], s=90, marker="D", color="teal", zorder=5)
    ax.annotate(
        "Prediction",
...
def main():
    ...
    # Softer dark theme + accent color
    st.markdown(
        """
        <style>
        .main {
    ...
    with col_right:
        ...
        with st.form("sustainable_form"):
            ...
            role = st.text_area(
                "Role",
                value="You are an AI assistant helping ML engineers design energy-efficient LLM workflows.",
                height=70,
            )
            context = st.text_area(
                "Context",
                value="Our team is building a customer-support chatbot using a large language model hosted in the cloud.",
                height=70,
            )
            expectations = st.text_area(
                "Expectations",
                value="Suggest a concise prompt template that reduces token usage without harming answer quality.",
                height=70,
            )
            ...
            with pcol1:
                num_layers = st.number_input(
                    "# Layers", min_value=1, max_value=2000, value=4, step=1
                )
            with pcol2:
